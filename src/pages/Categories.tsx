...
const Categories: React.FC = () => {
  ...
  const [color, setColor] = useState('#B98A4F')
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [catList, recList] = await Promise.all([getCategories(), getRecipes()])
      ...
      toast({
        ...
        description: 'Não foi possível buscar as categorias do acervo.',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const confirmDelete = (cat: Category) => {
    const count = recipeCountMap[cat.id] || 0
    if (count > 0) {
      toast({
        title: 'Exclusão bloqueada',
        description: `Esta categoria possui ${count} receita(s) vinculada(s). Reatribua-as antes de excluir.`,
        variant: 'destructive',
      })
      return
    }
  ...
  const presetColors = [
    '#B98A4F', // bronze
    '#2F4B3A', // verde profundo
    '#C29A3B', // dourado/mostarda
    '#B4553F', // terracota/vermelho
    '#4C7A5B', // verde sálvia
    '#8E5A78', // amora/vinho
    '#5A7D8E', // azul acinzentado
    '#8C6D4F', // café
  ]
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-marfim-border">
        <div>
          <span className="label-caps block mb-1">Classificação Culinária</span>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-marfim-border shadow-card">
          ...
          <p className="text-sm font-serif italic text-tinta-sec">Carregando categorias...</p>
        </div>
      ) : filteredCategories.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-marfim-border shadow-card">
          <FolderTree className="w-12 h-12 text-tinta-ter mx-auto mb-3" />
          ...
          <p className="text-sm text-tinta-sec max-w-md mx-auto mt-1 mb-6">
            Não localizamos categorias com o termo pesquisado.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredCategories.map((cat) => {
            const count = recipeCountMap[cat.id] || 0
            const customColor = cat.color || '#B98A4F'
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-tinta-sec line-clamp-3 mt-2 leading-relaxed">
                    {cat.description || 'Sem descrição cadastrada para esta seção.'}
                  </p>
                </div>
